...
export const columns: ColumnDef<StoreProductType>[] = [
  ...
  },
  {
    ...
    header: "SubCategory",
    cell: ({ row }) => {
      return <span>{row.original.subCategoryId.name}</span>;
    },
  },
  {
    accessorKey: "brand",
...

  {
    ...
    cell: ({ row }) => {
      return (
        <Link
          href={`/dashboard/seller/stores/${row.original.storeId.url}/products/${row.original._id}/variants/new`}
        >
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <CellActions productId={rowData._id} />;
    },
  },
];
