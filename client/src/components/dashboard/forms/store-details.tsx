  ...
  const handleSubmit = async (values: z.infer<typeof StoreFormSchema>) => {
    ...
    try {
      const storeData: Store = {
        ...
        phone: values.phone ?? form.getValues("phone"),
      ...
      if (data?._id) {
        storeData._id = data._id; // send _id only for update
      }
      ...
      const response = await upsertStore(storeData, token);

      // Displaying success message
      toast({
        title: data?._id
          ? "Store has been updated."
          : `Congratulations! '${response?.name}' is now createdstore.`,
      });
